...
| `parallel-tool-use` | COVERED | Several `tool_use` blocks in one turn; `disable_parallel_tool_use`. | [`examlab/parallel_tool_use.py`](../src/examlab/parallel_tool_use.py) for the block plumbing — one user message carrying every result, including the failed one. [`tools_mcp/parallel_tools.py`](../src/playground/tools_mcp/parallel_tools.py) for timing at the SDK layer, where grouping was a null result. This row was COVERED on the null result alone until 2026-08-21. The switch is API-ONLY. |
